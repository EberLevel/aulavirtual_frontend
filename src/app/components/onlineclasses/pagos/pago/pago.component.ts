...
export class PagoComponent implements OnInit {
  ...
  listarPagoVinculadoPorAlumno(): void {
    this.pagoService.listarPagoVinculadoPorAlumno(this.domain_id, this.selectedPago.id).subscribe((response: any) => {
  ...
  applyFilters2(): void {
    ...
    // Filtrar por ciclo si está seleccionado
    if (this.selectedCiclo) {
      this.filteredPagoAlumnos = this.filteredPagoAlumnos.filter(alumno => alumno.ciclo_id === this.selectedCiclo);
    }
    ...
    if (this.searchText && this.searchText.trim() !== '') {
      ...
      this.filteredPagoAlumnos = this.filteredPagoAlumnos.filter(alumno =>
        alumno.nombres?.toLowerCase().includes(searchTextLower) ||
        alumno.codigo?.toLowerCase().includes(searchTextLower)
      );
    }
  ...
  validatePayment(alumno: any): void {
    ...
    if (this.selectedAlumno.voucher_pago === '' || this.selectedAlumno.voucher_pago === null) {
      return this.messageService.add({
        severity: 'error',
        summary: 'Error',
  ...
  confirmPayment(alumno: any): void {
    ...
    this.pagoService.confirmPayment(this.domain_id, alumno.id, this.selectedPago.id).subscribe(
      (response: any) => {
